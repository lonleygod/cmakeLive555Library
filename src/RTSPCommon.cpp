...
Boolean parseRTSPRequestString(char const* reqStr,
				   ...
				   char* resultLocationStr,
				   unsigned resultLocationStrMaxSize,
  ...
  if (!parseSucceeded) return False;
    ...
    // Look for "Location:" (optional, case insensitive), skip whitespace,
  // then read everything up to the next \r or \n as 'Location':
 // parseSucceeded = False;
 // for (j = i; (int)j < (int)(reqStrSize-9); ++j) {
 //   if (_strncasecmp("Location:", &reqStr[j], 9) == 0) {
 //     j += 9;
 //     while (j < reqStrSize && (reqStr[j] ==  ' ' || reqStr[j] == '\t')) ++j;
 //     unsigned n;
 //     for (n = 0; n < resultLocationStrMaxSize-1 && j < reqStrSize; ++n,++j) {
	//char c = reqStr[j];
	//if (c == '\r' || c == '\n') {
	//  parseSucceeded = True;
	//  break;
	...
	//resultLocationStr[n] = c;
 //     }
 //     resultLocationStr[n] = '\0';
 //     break;
 //   }
 // }
 // if (!parseSucceeded) return False;
    ...
    // Look for "Location:" (optional, case insensitive), skip whitespace,
  // then read everything up to the next \r or \n as 'Location':
  resultLocationStr[0] = '\0'; // default value (empty string)
  for (j = i; (int)j < (int)(reqStrSize-9); ++j) {
    if (_strncasecmp("Location:", &reqStr[j], 9) == 0) {
      j += 9;
      while (j < reqStrSize && (reqStr[j] ==  ' ' || reqStr[j] == '\t')) ++j;
