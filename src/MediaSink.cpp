...
void MediaSink::SetOnDarwinClosure(afterPlayingFunc* afterFunc,
				void* afterClientData) 
{
	fAfterDarwinFunc = afterFunc;
	fAfterDarwinClientData = afterClientData;
}
...
void MediaSink::onDarwinClosure(void* clientData){
  MediaSink* sink = (MediaSink*)clientData;
  sink->fSource = NULL;
  if(sink->fAfterDarwinFunc != NULL ){
	  (*(sink->fAfterDarwinFunc))(sink->fAfterDarwinClientData);
  }
