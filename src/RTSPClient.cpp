...
RTSPClient* RTSPClient::createNew(UsageEnvironment& env, char const* rtspURL,
  ...
  return new RTSPClient(env, rtspURL, puid,
...
RTSPClient::RTSPClient(UsageEnvironment& env, char const* rtspURL, char const* puid,
  ...
  : Medium(env),
    ...
    fBaseURL(NULL),fPuid(NULL),fTCPStreamIdCount(0), fLastSessionId(NULL), fSessionTimeoutParameter(0),
	fSessionCookieCounter(0), fHTTPTunnelingConnectionIsPending(False),fRedirectHandler(NULL),fPauseHandler(NULL) {
  setBaseURL(rtspURL, puid);
...
void RTSPClient::setBaseURL(char const* url, char const* puid) {
  if(url != NULL)
	delete[] fBaseURL; fBaseURL = strDup(url);
  if(puid != NULL)
	delete[] fPuid; fPuid = strDup(puid);
}


//Boolean parseId(char const* reqUrl,
//			       unsigned reqUrlSize,
//			       char* resultId,
//			       unsigned resultIdMaxSize,
  ...
  do {
    ...
    if (strcmp(request->commandName(), "DESCRIBE") == 0) {
      extraHeaders = (char*)"Accept: application/sdp\r\n";
		...
		char const* extraHeaderFmt = "x-Module-ID: %s\r\n";
		unsigned extraHeaderSize = strlen(extraHeaderFmt) + 32;
		extraHeaders = new char[extraHeaderSize];
		sprintf(extraHeaders, extraHeaderFmt, puid());
...
void RTSPClient::handleIncomingRequest() {
  ...
  char location[RTSP_PARAM_STRING_MAX];
  ...
  //    }
	  ...
	  char buf[2*RTSP_PARAM_STRING_MAX] = { 0 };
	  unsigned bufLen = 0;
	  if(strcmp(cmdName, "REDIRECT") == 0)
	  {
		  (*fRedirectHandler)(buf, bufLen, cseq, location);
	  }
	  else if(strcmp(cmdName, "PAUSE") == 0)
	  {
		  (*fPauseHandler)(buf, bufLen, cseq, 0);
	  }
	  int len = send(fOutputSocketNum, buf, strlen(buf), 0);
	  //envir() << "RTSPClient Send:" << len << "Bytes Socket\n";
    return;
  } else {
    if (fVerbosityLevel >= 1) {
...
void RTSPClient::handleResponseBytes(int newBytesRead) {
  ...
  do {
	...
	if (checkForHeader(lineStart, "CSeq:", 5, headerParamsStr)) {
	  ...
	  setBaseURL(headerParamsStr, NULL);
